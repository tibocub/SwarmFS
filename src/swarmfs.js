...
export class SwarmFS {
  ...
  setupProtocolHandlers() {
    ...
    // When download starts, create progress bar
    this.protocol.on('chunk:download-started', async (info) => {
      if (!ProgressBar) {
        const progress = await import('./progress.js');
        ProgressBar = progress.ProgressBar;
      }
      currentProgressBar = new ProgressBar(info.size, 'Downloading');
      currentProgressBar.update(0);
    });

    // Update progress bar
    this.protocol.on('chunk:progress', (info) => {
      if (currentProgressBar) {
        currentProgressBar.update(info.current);
      }
    });
   ...
   */
  async downloadFile(topicName, merkleRoot, outputPath) {
    if (!this.protocol) {
      throw new Error('Not connected to network. Join a topic first.');
    }
