...
export default class Database {
  ...
  _get(sql, params) {
    ...
    // SELECT * FROM files WHERE merkle_root = ?
    if (normalizedSql.includes('select *') && normalizedSql.includes('files') && normalizedSql.includes('merkle_root')) {
      const [merkle_root] = params;
      return this.tables.files.find(f => f.merkle_root === merkle_root);
    }
